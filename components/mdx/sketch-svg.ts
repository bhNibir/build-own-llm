...
function forceLabelColor(root: Element, color: string) {
  ...
  // foreignObject HTML from SVG DOMParser is often not HTMLElement — don't use instanceof
  root.querySelectorAll('span, p, div, a, label, .nodeLabel, .edgeLabel, .label').forEach((el) => {
    ...
    el.setAttribute('style', `${cleaned};color:${color} !important;-webkit-text-fill-color:${color} !important;`);
  });
...
export function enhanceMermaidSvg(svg: string, isDark = false): string {
  ...
  try {
    ...
    root.querySelectorAll('g.node').forEach((node) => {
      ...
      const fill = patternUrl(resolved.fill, resolved.palette) ?? palette.bg;
