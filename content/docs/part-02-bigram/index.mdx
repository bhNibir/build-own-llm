---
title: Module 2 — Bigram LM
description: তোমার প্রথম Language Model — train করে text generate করো
---

# Part 2: Bigram Language Model

Part 1-এ আমরা data prepare করলাম — corpus থেকে training pair পর্যন্ত। এখন সেই pair দিয়ে **প্রথম সত্যিকারের Language Model** বানাবো। এটা পুরো GPT নয় — **Bigram Language Model**। কিন্তু GPT-এর foundation বুঝার জন্য এটা সবচেয়ে গুরুত্বপূর্ণ: আগের word দেখে পরেরটা guess করা।

## এই Part-এ কী শিখবে?

Count থেকে generate পর্যন্ত একটা ছোট loop:

```mermaid
flowchart LR
    A["Training Pairs"]:::input --> B["Count Table"]:::process
    B --> C["Predict"]:::output
    C --> D["Generate"]:::process
    D --> E["Sampling"]:::output
    classDef input fill:#EEF2FF,color:#1E1B4B,stroke:#4F46E5,stroke-width:2px
    classDef process fill:#ECFDF5,color:#064E3B,stroke:#059669,stroke-width:2px
    classDef output fill:#FFFBEB,color:#78350F,stroke:#D97706,stroke-width:2px
```

| Lesson | Topic |
|--------|-------|
| [01](/docs/part-02-bigram/01-bigram-concept) | Bigram কী? |
| [02](/docs/part-02-bigram/02-count-model) | Count-based Training |
| [03](/docs/part-02-bigram/03-predict) | Predict (argmax) |
| [04](/docs/part-02-bigram/04-generate) | Sentence Generate |
| [05](/docs/part-02-bigram/05-sampling-vs-argmax) | Sampling vs Argmax |

প্রতিটি code lesson-এ **interactive Playground** আছে — browser-এ edit করে run করো।

## মনে রাখো

> Bigram মানে: **একটা word দেখে পরেরটা guess করা**। কোনো neural network নেই — শুধু counting। কিন্তু idea same: training data থেকে pattern শেখা, তারপর next token predict।

এখান থেকেই শুরু — আগে concept, তারপর count table।

<Cards>
  <Card title="Lesson 01: Bigram Concept" href="/docs/part-02-bigram/01-bigram-concept" />
</Cards>
